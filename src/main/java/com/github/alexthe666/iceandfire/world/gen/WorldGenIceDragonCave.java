...
public class WorldGenIceDragonCave extends Feature<NoneFeatureConfiguration> {
    ...
    public WorldGenIceDragonCave(Codec<NoneFeatureConfiguration> configFactoryIn) {
    ...
    public static void setGoldPile(LevelAccessor world, BlockPos pos, Random rand) {
        ...
        if (!(world.getBlockState(pos).getBlock() instanceof BaseEntityBlock)) {
            if (chance < 60) {
                ...
                world.setBlock(pos, generateGold ? IafBlockRegistry.SILVER_PILE.defaultBlockState().setValue(BlockGoldPile.LAYERS, 1 + rand.nextInt(7)) : Blocks.AIR.defaultBlockState(), 3);
            } else if (chance == 61) {
                ...
                if (world.getBlockState(pos).getBlock() instanceof ChestBlock) {
                    ...
                    if (tileentity1 instanceof ChestBlockEntity) {
                        ((ChestBlockEntity) tileentity1).setLootTable(isMale ? ICEDRAGON_MALE_CHEST : ICEDRAGON_CHEST, rand.nextLong());
                    }
                }
            }
        }
    ...
    public boolean place(WorldGenLevel worldIn, ChunkGenerator p_230362_3_, Random rand, BlockPos position, NoneFeatureConfiguration p_230362_6_) {
        ...
        sphereList.clear();
        EntityIceDragon dragon = new EntityIceDragon(IafEntityRegistry.ICE_DRAGON, worldIn.getLevel());
                ...
                if (!(worldIn.getBlockState(position).getBlock() instanceof BaseEntityBlock) && worldIn.getBlockState(position).getDestroySpeed( worldIn, position) >= 0) {
                    ...
                    if (doOres) {
                        ...
                        if (chance > 40 && chance < 45) {
                            worldIn.setBlock(blockPos, IafConfig.generateCopperOre ? IafBlockRegistry.COPPER_ORE.defaultBlockState() : IafBlockRegistry.FROZEN_STONE.defaultBlockState(), 3);
                        }
                        if (chance > 45 && chance < 50) {
                            worldIn.setBlock(blockPos, IafConfig.generateSilverOre ? IafBlockRegistry.SILVER_ORE.defaultBlockState() : IafBlockRegistry.FROZEN_STONE.defaultBlockState(), 3);
                        }
                        ...
                        if (chance > 90 && chance < 1000) {
                            worldIn.setBlock(blockPos, IafConfig.generateSapphireOre ? IafBlockRegistry.SAPPHIRE_ORE.defaultBlockState() : Blocks.EMERALD_ORE.defaultBlockState(), 3);
                        }
                    } else {
                        worldIn.setBlock(blockPos, rand.nextBoolean() ? IafBlockRegistry.FROZEN_COBBLESTONE.defaultBlockState() : IafBlockRegistry.FROZEN_STONE.defaultBlockState(), 2);
                    }
                }
            }
        });
