...
public class WorldGenCyclopsCave extends Feature<NoneFeatureConfiguration> {
    ...
    private void genSheepPen(ServerLevelAccessor worldIn, BlockPos blockpos, Random rand, BlockPos origin, float radius) {
        ...
        for (int sideCount = 0; sideCount < 4; sideCount++) {
            for (int side = 0; side < width; side++) {
                if (origin.distSqr(end.relative(direction, side)) <= (double) (radius * radius)) {
        ...
        for (int x = 1; x < width - 1; x++) {
            for (int z = 1; z < width - 1; z++) {
                if (origin.distSqr(end.offset(x, 0, z)) <= (double) (radius * radius)) {
    ...
    public boolean place(WorldGenLevel worldIn, ChunkGenerator p_230362_3_, Random rand, BlockPos position, NoneFeatureConfiguration p_230362_6_) {
        ...
        }
        {
            ...
            float f = (float) (j + k + l) * 0.333F + 0.5F;
            for (BlockPos blockpos : BlockPos.betweenClosedStream(position.offset(-j, -k, -l), position.offset(j, k, l)).map(BlockPos::immutable).collect(Collectors.toSet())) {
                if (blockpos.distSqr(position) <= (double) (f * f) && blockpos.getY() > position.getY()) {
        ...
        EntityCyclops cyclops = new EntityCyclops(IafEntityRegistry.CYCLOPS, worldIn.getLevel());
