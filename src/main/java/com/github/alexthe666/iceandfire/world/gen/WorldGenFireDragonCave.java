...
public class WorldGenFireDragonCave extends Feature<NoneFeatureConfiguration> {
    ...
    public static void setGoldPile(LevelAccessor world, BlockPos pos, Random rand) {
        ...
        if (!(world.getBlockState(pos).getBlock() instanceof BaseEntityBlock)) {
            if (chance < 60) {
                ...
                world.setBlock(pos, generateGold ? IafBlockRegistry.GOLD_PILE.defaultBlockState().setValue(BlockGoldPile.LAYERS, 1 + rand.nextInt(7)) : Blocks.AIR.defaultBlockState(), 3);
            } else if (chance == 61) {
                ...
                if (world.getBlockState(pos).getBlock() instanceof ChestBlock) {
                    ...
                    if (tileentity1 instanceof ChestBlockEntity) {
                        ((ChestBlockEntity) tileentity1).setLootTable(isMale ? FIREDRAGON_MALE_CHEST : FIREDRAGON_CHEST, rand.nextLong());
                    }
                }
            }
        }
    ...
    public boolean place(WorldGenLevel worldIn, ChunkGenerator p_230362_3_, Random rand, BlockPos position, NoneFeatureConfiguration p_230362_6_) {
        ...
        sphereList.clear();
        EntityFireDragon dragon = new EntityFireDragon(IafEntityRegistry.FIRE_DRAGON, worldIn.getLevel());
                ...
                if (!(worldIn.getBlockState(position).getBlock() instanceof BaseEntityBlock) && worldIn.getBlockState(position).getDestroySpeed(worldIn, position) >= 0) {
                    ...
                    if (doOres) {
                        ...
                        if (chance < 30) {
                            ...
                            worldIn.setBlock(blockPos, IafConfig.generateCopperOre ? IafBlockRegistry.COPPER_ORE.defaultBlockState() : IafBlockRegistry.CHARRED_STONE.defaultBlockState(), 3);
                        }
                        else if (chance > 45 && chance < 50) {
                            worldIn.setBlock(blockPos, IafConfig.generateSilverOre ? IafBlockRegistry.SILVER_ORE.defaultBlockState() : IafBlockRegistry.CHARRED_STONE.defaultBlockState(), 3);
                        }
                            ...
                            worldIn.setBlock(blockPos, Blocks.EMERALD_ORE.defaultBlockState(), 3);
                        }
                    } else {
                        worldIn.setBlock(blockPos, rand.nextBoolean() ? IafBlockRegistry.CHARRED_COBBLESTONE.defaultBlockState() : IafBlockRegistry.CHARRED_STONE.defaultBlockState(), 2);
                    }
                }
            }
        });
