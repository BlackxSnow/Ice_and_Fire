...
public class GuiMyrmexAddRoom extends Screen {
    ...
    protected void init() {
        ...
        if (ClientProxy.getReferedClientHive() != null) {
            ...
            this.addButton(new Button(i + 50, j + 35, 150, 20, new TranslatableComponent("myrmex.message.establishroom_food"), (p_214132_1_) -> {
                ClientProxy.getReferedClientHive().addRoomWithMessage(player, interactPos, WorldGenMyrmexHive.RoomType.FOOD);
                onGuiClosed();
                Minecraft.getInstance().setScreen(null);
            }));
            this.addButton(new Button(i + 50, j + 60, 150, 20, new TranslatableComponent("myrmex.message.establishroom_nursery"), (p_214132_1_) -> {
                ClientProxy.getReferedClientHive().addRoomWithMessage(player, interactPos, WorldGenMyrmexHive.RoomType.NURSERY);
                onGuiClosed();
                Minecraft.getInstance().setScreen(null);
            }));
            this.addButton(new Button(i + 50, j + 85, 150, 20, new TranslatableComponent("myrmex.message.establishroom_enterance_surface"), (p_214132_1_) -> {
                ClientProxy.getReferedClientHive().addEnteranceWithMessage(player, false, interactPos, facing);
                onGuiClosed();
                Minecraft.getInstance().setScreen(null);

            }));
            this.addButton(new Button(i + 50, j + 110, 150, 20, new TranslatableComponent("myrmex.message.establishroom_enterance_bottom"), (p_214132_1_) -> {
                ClientProxy.getReferedClientHive().addEnteranceWithMessage(player, true, interactPos, facing);
                onGuiClosed();
                Minecraft.getInstance().setScreen(null);

            }));
            this.addButton(new Button(i + 50, j + 135, 150, 20, new TranslatableComponent("myrmex.message.establishroom_misc"), (p_214132_1_) -> {
