...
public class RenderPodium<T extends TileEntityPodium> extends BlockEntityRenderer<T> {

    public RenderPodium(BlockEntityRenderDispatcher rendererDispatcherIn) {
    ...
    public void render(T entity, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (!podium.getItem(0).isEmpty()) {
            if (podium.getItem(0).getItem() instanceof ItemDragonEgg) {
                ...
                Minecraft.getInstance().getItemRenderer().renderStatic(podium.getItem(0), ItemTransforms.TransformType.FIXED, combinedLightIn, combinedOverlayIn, matrixStackIn, bufferIn);
                matrixStackIn.popPose();
                matrixStackIn.popPose();
                //}
            }
