...
public class EntityDragonIceCharge extends Fireball implements IDragonProjectile {

    public int ticksInAir;

    public EntityDragonIceCharge(EntityType type, Level worldIn) {
        super(type, worldIn);
    ...
    public EntityDragonIceCharge(EntityType type, Level worldIn, double posX, double posY, double posZ, double accelX, double accelY, double accelZ) {
    ...
    public EntityDragonIceCharge(EntityType type, Level worldIn, EntityDragonBase shooter, double accelX, double accelY, double accelZ) {
    ...
    public void tick() {
        ...
        if (this.level.isClientSide) {
            for (int i = 0; i < 14; ++i) {
                IceAndFire.PROXY.spawnParticle("dragonice", this.getX() + this.random.nextDouble() * 1 * (this.random.nextBoolean() ? -1 : 1), this.getY() + this.random.nextDouble() * 1 * (this.random.nextBoolean() ? -1 : 1), this.getZ() + this.random.nextDouble() * 1 * (this.random.nextBoolean() ? -1 : 1), 0.0D, 0.0D, 0.0D);
            }
        }
        if (this.level.isClientSide || (shootingEntity == null || shootingEntity.isAlive()) && this.level.hasChunkAt(this.blockPosition())) {
            ...
            if (!level.isClientSide && raytraceresult != null) {
    ...
    protected void onHit(HitResult movingObject) {
            ...
            if (movingObject.getType() == HitResult.Type.ENTITY) {
                ...
                if (entity != null && !(entity instanceof IDragonProjectile) && !entity.is(shootingEntity)) {
                    ...
                    if (shootingEntity != null && shootingEntity instanceof EntityDragonBase) {
                        ...
                        entity.hurt(IafDamageRegistry.DRAGON_ICE, damageAmount);
        ...
        if(movingObject.getType() != HitResult.Type.MISS) {
            if (shootingEntity instanceof EntityDragonBase && IafConfig.dragonGriefing != 2 && !this.isInWater()) {
                IafDragonDestructionManager.destroyAreaIceCharge(level, this.blockPosition(), ((EntityDragonBase) shootingEntity));
            }
            this.remove();
        }
    }
