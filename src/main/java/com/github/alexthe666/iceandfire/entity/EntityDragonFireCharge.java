...
public class EntityDragonFireCharge extends Fireball implements IDragonProjectile {

    public int ticksInAir;

    public EntityDragonFireCharge(EntityType type, Level worldIn) {
        super(type, worldIn);
    }

    public EntityDragonFireCharge(FMLPlayMessages.SpawnEntity spawnEntity, Level worldIn) {
        this(IafEntityRegistry.FIRE_DRAGON_CHARGE, worldIn);
    }

    public EntityDragonFireCharge(EntityType type, Level worldIn, double posX, double posY, double posZ, double accelX, double accelY, double accelZ) {
    ...
    protected void onHit(HitResult movingObject) {
            ...
            if (movingObject.getType() == HitResult.Type.ENTITY) {
                ...
                if (entity != null && !(entity instanceof IDragonProjectile) && !entity.is(shootingEntity)) {
                    ...
                    if (shootingEntity != null && shootingEntity instanceof EntityDragonBase) {
                        ...
                        entity.hurt(IafDamageRegistry.DRAGON_FIRE, damageAmount);
        ...
        if(movingObject.getType() != HitResult.Type.MISS) {
            if (shootingEntity instanceof EntityDragonBase && IafConfig.dragonGriefing != 2) {
                IafDragonDestructionManager.destroyAreaFireCharge(level, this.blockPosition(), ((EntityDragonBase) shootingEntity));
            }
            this.remove();
        }
    }
