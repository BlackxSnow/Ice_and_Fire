...
public class DragonUtils {
    ...
    public static BlockPos getBlockInView(EntityDragonBase dragon) {
        ...
        if (dragon.hasHomePosition && dragon.homePos != null) {
            ...
            for (int i = 0; i < 10; i++) {
                BlockPos pos = new BlockPos(dragon.homePos.getX() + dragon.getRandom().nextInt(IafConfig.dragonWanderFromHomeDistance * 2) - IafConfig.dragonWanderFromHomeDistance, (distFromGround > 16 ? (int) Math.min(IafConfig.maxDragonFlight, dragon.getY() + dragon.getRandom().nextInt(16) - 8) : (int) dragon.getY() + dragon.getRandom().nextInt(16) + 1), (dragon.homePos.getZ() + dragon.getRandom().nextInt(IafConfig.dragonWanderFromHomeDistance * 2) - IafConfig.dragonWanderFromHomeDistance));
                if (!dragon.isTargetBlocked(Vec3.atCenterOf(pos)) && dragon.getDistanceSquared(Vec3.atCenterOf(pos)) > 6) {
                    return pos;
                }
            }
    ...
    public static LivingEntity riderLookingAtEntity(LivingEntity dragon, LivingEntity rider, double dist) {
        ...
        for (int j = 0; j < list.size(); ++j) {
            ...
            Vec3 raytraceresult = axisalignedbb.clip(Vector3d, Vector3d2).orElse(Vector3d.ZERO);
    ...
    public static BlockPos getBlockInViewStymphalian(EntityStymphalianBird bird) {
        ...
        int flightHeight = Math.min(IafConfig.stymphalianBirdFlightHeight, bird.flock != null && !bird.flock.isLeader(bird) ? ground.getY() + bird.getRandom().nextInt(16) : ground.getY() + bird.getRandom().nextInt(16));
    ...
    private static float getStymphalianFlockDirection(EntityStymphalianBird bird) {
        ...
        if (bird.distanceToSqr(leader) > 2) {
            ...
            double d1 = leader.getY() + (double) leader.getEyeHeight() - (bird.getY() + (double) bird.getEyeHeight());
            // Note: Was not cast before. Loss of precision ?
    ...
    public static BlockPos getBlockInTargetsViewCockatrice(EntityCockatrice cockatrice, LivingEntity target) {
        ...
        float neg = cockatrice.getRandom().nextBoolean() ? 1 : -1;
    ...
    public static BlockPos getBlockInTargetsViewGhost(EntityGhost ghost, LivingEntity target) {
        ...
        float neg = ghost.getRandom().nextBoolean() ? 1 : -1;
    ...
    public static BlockPos getBlockInTargetsViewSeaSerpent(EntitySeaSerpent serpent, LivingEntity target) {
        ...
        float neg = serpent.getRandom().nextBoolean() ? 1 : -1;
        float angle = (0.01745329251F * target.yHeadRot);
        double extraX = radius * Mth.sin((float) (Math.PI + angle));
        double extraZ = radius * Mth.cos(angle);
        BlockPos radialPos = new BlockPos(target.getX() + extraX, 0, target.getZ() + extraZ);
    ...
    public static boolean canTameDragonAttack(TamableAnimal dragon, Entity entity) {
        if (entity.getType().is(IafTagRegistry.VILLAGERS)) {
            return false;
        }
    ...
    public static boolean isVillager(Entity entity) {
        return entity.getType().is(IafTagRegistry.VILLAGERS);
    }
