...
public class PixieAIFlee<T extends Entity> extends Goal {
    ...
    public PixieAIFlee(EntityPixie pixie, Class<T> classToAvoidIn, float avoidDistanceIn, Predicate<? super T> avoidTargetSelectorIn) {
        ...
        this.canBeSeenSelector = new Predicate<Entity>() {
            public boolean apply(@Nullable Entity entity) {
                return entity.isAlive() && PixieAIFlee.this.pixie.getSensing().canSee(entity) && !PixieAIFlee.this.pixie.isAlliedTo(entity);
            }
        };
    ...
    public boolean canUse() {
        if (this.pixie.getItemInHand(InteractionHand.MAIN_HAND).isEmpty()) {
            return false;
        }
        if (this.pixie.isTame()) {
            return false;
        }
        List<T> list = this.pixie.level.getEntitiesOfClass(this.classToAvoid, this.pixie.getBoundingBox().inflate(this.avoidDistance, 3.0D, this.avoidDistance),
