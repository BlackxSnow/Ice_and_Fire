...
public class EntityGhost extends Monster implements IAnimatedEntity, IVillagerFear, IAnimalFear, IHumanoid, IBlacklistedFromStatues {
    ...
    public EntityGhost(EntityType type, Level worldIn) {
        super(type, worldIn);
    ...
    public void aiStep() {
        ...
        if(!level.isClientSide){
            ...
            if(this.getAnimation() == ANIMATION_SCARE && this.getAnimationTick() == 3 && !this.isHauntedShoppingList() && random.nextInt(3) == 0){
                ...
                if(level.isClientSide){
                    IceAndFire.PROXY.spawnParticle("ghost_appearance", this.getX(), this.getY(), this.getZ(), this.getId(), 0, 0);
                }
            }
        }
            ...
            if (this.operation == Operation.MOVE_TO) {
                ...
                if (d0 < edgeLength) {
                    ...
                    if (ghost.getTarget() == null) {
                        ...
                        ghost.yRot = -((float) Mth.atan2(vec3d1.x, vec3d1.z)) * (180F / (float) Math.PI);
                        ghost.yBodyRot = ghost.yRot;
                    } else {
                        double d4 = ghost.getTarget().getX() - ghost.getX();
                        double d5 = ghost.getTarget().getZ() - ghost.getZ();
                        ghost.yRot = -((float) Mth.atan2(d4, d5)) * (180F / (float) Math.PI);
                        ghost.yBodyRot = ghost.yRot;
                    }
                }
            }
