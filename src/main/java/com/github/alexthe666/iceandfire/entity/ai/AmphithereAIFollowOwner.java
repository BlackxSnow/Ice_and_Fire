...
public class AmphithereAIFollowOwner extends Goal {
    ...
    public boolean canUse() {
        ...
        if (LivingEntity == null) {
            ...
            return false;
        } else if (this.ampithere.distanceToSqr(LivingEntity) < (double) (this.minDist * this.minDist)) {
            return false;
        } else {
    ...
    public boolean canContinueToUse() {
        return !noPath() && this.ampithere.distanceToSqr(this.owner) > (double) (this.maxDist * this.maxDist) && !this.ampithere.isOrderedToSit();
    }
    ...
    public void tick() {
        this.ampithere.getLookControl().setLookAt(this.owner, 10.0F, (float) this.ampithere.getMaxHeadXRot());
                        ...
                        for (int l = 0; l <= 4; ++l) {
                            for (int i1 = 0; i1 <= 4; ++i1) {
                                if ((l < 1 || i1 < 1 || l > 3 || i1 > 3) && this.canTeleportToBlock(new BlockPos(i, j, k))) {
                                    this.ampithere.moveTo((float) (i + l) + 0.5F, k, (float) (j + i1) + 0.5F, this.ampithere.yRot, this.ampithere.xRot);
