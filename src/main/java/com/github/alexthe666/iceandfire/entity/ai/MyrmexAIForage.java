...
public class MyrmexAIForage extends Goal {
    ...
    public boolean canUse() {
        if (!this.myrmex.canMove() || this.myrmex.holdingSomething() || !this.myrmex.getNavigation().isDone() || this.myrmex.isInHive() || this.myrmex.shouldEnterHive()) {
            return false;
        }
        ...
        this.path = ((AdvancedPathNavigate) this.myrmex.getNavigation()).moveToXYZ(targetBlock.getX(), targetBlock.getY(), targetBlock.getZ(), 1D);
        return targetBlock != null && myrmex.getRandom().nextInt(chance) == 0;
    }
        ...
        if (targetBlock != null && this.myrmex.keepSearching) {
            ...
            if (EntityMyrmexBase.isEdibleBlock(block)) {
                ...
                if (distance < 6) {
                    ...
                    List<ItemStack> drops = block.getBlock().getDrops(block, (ServerLevel) this.myrmex.level, this.targetBlock, this.myrmex.level.getBlockEntity(targetBlock)); // use the old method until it gets removed, for backward compatibility
                    if (!drops.isEmpty()) {
                        ...
                        for (ItemStack stack : drops) {
                            ItemEntity itemEntity = new ItemEntity(this.myrmex.level, this.targetBlock.getX() + this.myrmex.getRandom().nextDouble(), this.targetBlock.getY() + this.myrmex.getRandom().nextDouble(), this.targetBlock.getZ() + this.myrmex.getRandom().nextDouble(), stack);
    ...
    private double getDistanceSq(BlockPos pos) {
        double deltaX = this.myrmex.getX() - (pos.getX() + 0.5);
        double deltaY = this.myrmex.getY() + this.myrmex.getEyeHeight() - (pos.getY() + 0.5);
        double deltaZ = this.myrmex.getZ() - (pos.getZ() + 0.5);
    ...
    private List<BlockPos> getEdibleBlocks(){
        ...
        BlockPos.betweenClosedStream(this.myrmex.blockPosition().offset(-RADIUS, -RADIUS/2, -RADIUS), this.myrmex.blockPosition().offset(RADIUS, RADIUS/2, RADIUS)).map(BlockPos::immutable).forEach(pos -> {
    ...
    private boolean increaseRadiusAndWander(){
        ...
        Vec3 vec = RandomPos.getPos(this.myrmex, wanderRadius, 7);
        if (vec != null) {
