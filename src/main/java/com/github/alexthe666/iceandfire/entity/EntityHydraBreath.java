...
public class EntityHydraBreath extends Fireball implements IDragonProjectile {
    ...
    public EntityHydraBreath(EntityType t, Level worldIn) {
        super(t, worldIn);
    }

    public EntityHydraBreath(EntityType t, Level worldIn, double posX, double posY, double posZ, double accelX, double accelY, double accelZ) {
    ...
    public EntityHydraBreath(EntityType t, Level worldIn, EntityHydra shooter, double accelX, double accelY, double accelZ) {
        ...
        if (this.level.isClientSide || (shootingEntity == null || shootingEntity.isAlive()) && this.level.hasChunkAt(this.blockPosition())) {
            if (this.level.isClientSide || (shootingEntity == null || !shootingEntity.removed) && this.level.hasChunkAt(this.blockPosition())) {
                ...
                if (this.level.isClientSide) {
                    for (int i = 0; i < 15; ++i) {
                        IceAndFire.PROXY.spawnParticle("hydra", this.getX() + (double) (this.random.nextFloat() * this.getBbWidth()) - (double) this.getBbWidth() * 0.5F, this.getY() - 0.5D, this.getZ() + (double) (this.random.nextFloat() * this.getBbWidth()) - (double) this.getBbWidth() * 0.5F, 0.1D, 1.0D, 0.1D);
                    }
                }
