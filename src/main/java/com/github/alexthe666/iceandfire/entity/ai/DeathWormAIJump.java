...
public class DeathWormAIJump extends JumpGoal {
    ...
    public boolean canUse() {
        ...
        if (this.dolphin.getRandom().nextInt(this.interval) != 0 || dolphin.isVehicle() || dolphin.getTarget() != null) {
            return false;
        } else {
    ...
    public boolean canContinueToUse() {
        ...
        return jumpCooldown > 0 && (!(d0 * d0 < (double) 0.03F) || this.dolphin.xRot == 0.0F || !(Math.abs(this.dolphin.xRot) < 10.0F) || !this.dolphin.isInSand()) && !this.dolphin.isOnGround();
    }
    ...
    public void start() {
        Direction direction = this.dolphin.getMotionDirection();
        float up = (dolphin.getScale() > 3 ? 0.7F : 0.4F) + dolphin.getRandom().nextFloat() * 0.4F;
