...
public class EntityIceDragon extends EntityDragonBase {
    ...
    public void aiStep() {
        ...
        if (!level.isClientSide && this.getTarget() != null) {
            ...
            if (this.getBoundingBox().inflate(0 + this.getRenderSize() * 0.33F, 0 + this.getRenderSize() * 0.33F, 0 + this.getRenderSize() * 0.33F).intersects(this.getTarget().getBoundingBox())) {
                doHurtTarget(this.getTarget());
            }
            if (this.groundAttack == IafDragonAttacks.Ground.FIRE && (usingGroundAttack || this.onGround)) {
                shootIceAtMob(this.getTarget());
            }
    ...
    public void riderShootFire(Entity controller) {
        if (this.getRandom().nextInt(5) == 0 && !this.isBaby()) {
            if (this.getAnimation() != ANIMATION_FIRECHARGE) {
                ...
                EntityDragonIceCharge entitylargefireball = new EntityDragonIceCharge(IafEntityRegistry.ICE_DRAGON_CHARGE, level, this, d2, d3, d4);
    ...
    protected void spawnDeathParticles() {
        if (this.level.isClientSide) {
            for (int k = 0; k < 10; ++k) {
                ...
                IceAndFire.PROXY.spawnParticle("snowflake", this.getX() + (double) (this.random.nextFloat() * this.getBbWidth() * 2.0F) - (double) this.getBbWidth(), this.getY() + (double) (this.random.nextFloat() * this.getBbHeight()), this.getZ() + (double) (this.random.nextFloat() * this.getBbWidth() * 2.0F) - (double) this.getBbWidth(), d2, d0, d1);
            }
        }
    }
